# Training the classification DNNs with CardioDefense
...
def train_model(train_loader, val_loader, NUM_EPOCHS, LEARNING_RATE, Temp, eps, step_alpha, num_steps, sizes, weights):
    ...
    file_name = 'ATdbest_initial_model_Temp'+str(Temp)+'_100epoch'
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print('AdversarialEpoch_T1: [{}/{}], Step: [{}/{}], Val Acc: {}, Val F1: {}, Train Acc: {}, Train Loss: {}'.format(epoch + 1, NUM_EPOCHS, i + 1, len(train_loader), val_acc, val_F1, train_acc, train_loss))
        sys.stdout.flush()
    print('#### End Training ####')
    print('best val acc:', best_acc)
    print('best F1:', best_F1)
    # init_val_accuracy = pd.DataFrame(val_accuracy_init, columns=['val_accuracy'])
    # init_val_accuracy.to_csv("./loss/1stAT_init5v5T1_1_100epochval_accuracy.csv",header=True)
...
def distilled_model(train_loader, val_loader, NUM_EPOCHS, LEARNING_RATE, Temp, eps, step_alpha, num_steps, sizes, weights):
    ...
    file_name = 'ATdbest_distilled_model_Temp'+str(Temp)+'_100epoch'
    model = CNN(num_classes=4)
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model, device_ids=[3,4])
    model = model.to(device)
    ...
    # dist_val_accuracy.to_csv("./loss/1stAT_dist5v5T1_1_100epochval_accuracy.csv",header=True)
...
if __name__ == "__main__":
    ...
    PADDING = 'two'
    Temp = 1  # Temperature parameter of CadioDefense
    ...
    model = nn.DataParallel(model, device_ids=[3,4])
    model = model.to(device)
    model.load_state_dict(torch.load('saved_model/ATdbest_initial_model_Temp'+str(Temp)+'_100epoch.pth', map_location=lambda storage, loc: storage))
    
    # create soft labels for the Distilled Network
    soft_labels = torch.zeros(train_label.shape)  # (number of samples,4)
    soft_data = torch.zeros((train_label.shape[0], 1, 9000))  #(number of samples, 1, 9000)
