# Training the classification DNNs with Init-CardioDefense
...
def train_model(train_loader, val_loader, NUM_EPOCHS, LEARNING_RATE, Temp, eps, step_alpha, num_steps, sizes, weights):
    ...
    file_name = 'iniATdbest_initial_model_Temp'+str(Temp)+'_100epoch'
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print('iniAdversarialEpoch: [{}/{}], Step: [{}/{}], Val Acc: {}, Val F1: {}, Train Acc: {}, Train Loss: {}'.format(epoch + 1, NUM_EPOCHS, i + 1, len(train_loader), val_acc, val_F1, train_acc, train_loss))
        sys.stdout.flush()
    print('#### End Training ####')
    print('best val acc:', best_acc)
    print('best F1:', best_F1)
    # init_val_accuracy = pd.DataFrame(val_accuracy_init, columns=['val_accuracy'])
    # init_val_accuracy.to_csv("./loss/iniAT_init5v5T1_70epochval_accuracy.csv",header=True)
...
def distilled_model(train_loader, val_loader, NUM_EPOCHS, LEARNING_RATE, Temp, eps, step_alpha, num_steps, sizes, weights):
    ...
    file_name = 'iniATdbest_distilled_model_Temp'+str(Temp)+'_100epoch'
    model = CNN(num_classes=4)  # 构建model类
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model, device_ids=[3,4])
    model = model.to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_acc > best_acc:
            ...
            best_F1 = val_F1
            torch.save(model.state_dict(), 'saved_model/'+file_name+'.pth')
        train_acc = test_model(train_loader, model)  # test_model主要用于计算准确率
    ...
    # dist_val_accuracy.to_csv("./loss/1stiniAT_dist5v5T1_70epochval_accuracy.csv",header=True)
...
if __name__ == "__main__":
    ...
    model = nn.DataParallel(model, device_ids=[3,4])
    model = model.to(device)
    model.load_state_dict(torch.load('saved_model/iniATdbest_initial_model_Temp'+str(Temp)+'_100epoch.pth', map_location=lambda storage, loc: storage))
